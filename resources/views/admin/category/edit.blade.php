...
@section('content')
  <!-- Page title -->
  <h1 class="text-2xl font-semibold tracking-wide text-gray-600">Editing a category</h1>
  ...
  <form action="{{route('categories.update', $category->id)}}" method="POST" class="space-y-5">
    ...
    <div class="space-y-2">
      ...
      <div class="w-3/5">
        ...
        <input type="text" name="name" value="@if(old('name')) {{old('name')}} @else {{$category->name}} @endif" class="w-full px-2 py-1 rounded-md placeholder-gray-300 outline-none focus:ring-0 text-gray-500 border border-gray-300 shadow">
        @error('name')
